...
contract Staker {
  ...
  */
        ...
        notCompleted
    {
        ...
        uint256 indBalanceRewards = individualBalance +
            ((2**(block.number - depositBlockNumber[msg.sender])) *
                rewardRatePerBlock);
  ...
  */
    ...
    function receiveFunds() public payable claimDeadlineReached(true) {
        address payable receiver = payable(address(this));
        exampleExternalContract.reTransfer(receiver);
    }

    function reDeploy() public payable claimDeadlineReached(true) {
        withdrawalDeadline = block.timestamp + 120 seconds;
        claimDeadline = block.timestamp + 240 seconds;
        exampleExternalContract.reverseComplete();
        balances[msg.sender] = 0;
